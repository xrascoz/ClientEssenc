...
function Card({ user, setUpdateUi }) {
    ...
    let [appointmentsLenght, setAppointmentsLenght] = useState([])
    
    
    useEffect(() => {
        setAppointmentsLenght(appointments)
    } , [])
    console.log(appointmentsLenght)
    ...
    let appointmentsAvailable = (e) => {
        let appointmentsId = e.currentTarget.getAttribute("appointmentsId")
        axios.put(`http://localhost:5000/api/user/appointments/${_id}/${appointmentsId}/`, { available: true }).then((response) => {
            console.log(response.data)
            if (response.data.error) {
                setErrorAlertMessage(response.data.error)
                setToggleAlertError(true)
